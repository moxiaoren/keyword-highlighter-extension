...
const Storage = {
  ...
  async getAll() {
    ...
    merged.keywords = await this._maybeFlipCombo(merged.keywords || []);
    return merged;
  },
  ...
  async getKeywords() {
    ...
    // 组合关键词翻转迁移(v1.8.3)：旧格式 text=左格词、cellVerify=右格期望值 →
    // 新格式 text=右格核心(关键词)、cellVerify=左格标题。交换两字段并标记 _flipped，幂等，自动写回。
    return this._maybeFlipCombo(keywords);
  },

  // 组合关键词「左右翻转」自动迁移（v1.8.3），幂等（已标记 _flipped 不再交换）
  async _maybeFlipCombo(keywords) {
    let changed = false;
    const next = keywords.map(k => {
      if (k && k.cellVerifyEnabled && k.cellVerify && !k._flipped) {
        const oldText = k.text;
        const oldCv = k.cellVerify;
        changed = true;
        return Object.assign({}, k, { text: oldCv, cellVerify: oldText, _flipped: true });
      }
      return k;
    });
    if (changed) await this.set({ keywords: next });
    return next;
  },
