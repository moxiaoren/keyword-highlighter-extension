...
const Utils = {
  ...
  sanitizeHTML(str) {
    ...
    // [text](url) → <a href="url" target="_blank">text</a>（v1.8.20：支持 URL 内含括号；非法协议保持原文不转链接）
    text = this._mdLinksToHTML(text, false);
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (allowedTags[tagName]) {
          ...
          if (tagName === 'a') {
            ...
            // v1.8.20 兜底：不安全的 href 协议（javascript: 等）去除 href，避免 XSS
            const href = node.getAttribute('href') || '';
            const hrefOk = !href || /^(https?:|mailto:|tel:|ftp:)/i.test(href) || /^[\/#]/.test(href) || /^(\.\.?\/)/.test(href);
            if (href && !hrefOk) node.removeAttribute('href');
          }
  ...
  /**
   * 把 [text](url) 链接 markdown 转成安全的 <a> HTML（v1.8.20）。
   * 特性：
   *  - 支持 URL 内含配对的括号（如 https://en.wikipedia.org/wiki/X_(film)），不再被闭括号截断；
   *  - 支持尾部可选 title（[x](url "说明")）；
   *  - 非法协议（javascript: 等）不转链接，保持原文；
   *  - escLabel=true 时对链接文字做 HTML 转义（供表格占位用，因为表格占位绕过 cleanNode 清洗）。
   */
  _mdLinksToHTML(text, escLabel) {
    const escapeAttr = (s) => String(s).replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const escapeHtml = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    // 合法协议 / 相对地址才转链接，其余保持原文（防 javascript: 等注入）
    const safeUrl = (u) => {
      u = (u || '').trim();
      if (!u) return null;
      if (u === '#') return '#';
    ...
    while ((m = re.exec(text))) {
      ...
      let k = re.lastIndex; // 指向 '(' 之后
      for (; k < text.length; k++) {
        const ch = text[k];
        if (ch === '(') depth++;
        else if (ch === ')') {
          if (depth === 0) { closed = k; break; }
          depth--;
        }
      }
      if (closed === -1) { out += m[0]; last = re.lastIndex; continue; }
      let rawUrl = text.slice(re.lastIndex, closed);
      // 去掉 url 尾部可选 title（"title" / 'title'）
      rawUrl = rawUrl.replace(/\s+["'][\s\S]*?["']\s*$/, '').trim();
      const label = m[1];
      const url = safeUrl(rawUrl);
      if (url === null) { out += text.slice(m.index, closed + 1); last = closed + 1; re.lastIndex = closed + 1; continue; }
      const lbl = escLabel ? escapeHtml(label) : label;
      out += '<a href="' + escapeAttr(url) + '" target="_blank" rel="noopener noreferrer">' + lbl + '</a>';
      last = closed + 1;
  ...
  _buildTable(block) {
    const lines = block.split('\n').map(l => l.trim()).filter(Boolean)
      // 剔除 markdown 分隔行（| --- |、|---|---| 等单/多列），避免渲染成一个带 --- 的单元格
      // （v1.8.0 起；v1.11.1 修复单列 | --- | 因 core 无 '|' 而 length=1 漏网的 bug）
      .filter(line => {
        const core = line.replace(/^\|/, '').replace(/\|$/, '').trim();
        const strip = core.replace(/[\s|:]/g, '');
        if (strip && /^-+$/.test(strip)) return false; // 分隔行：去空白/|/冒号后全为 '-'
        return true;
      });
    const rows = lines.map(line => {
      ...
      const cells = inner.split('|').map(c => {
        ...
        let cellHtml = d.innerHTML;
        // 单元格内图片 markdown → <img>（v1.8.12）。表格占位在 cleanNode 之后才还原、不再过安全清洗，
        // 故此处必须就地校验 src（仅 http/https/data:image），非法回退为原文，防止注入。
        cellHtml = cellHtml.replace(/!\[([^\]]*)\]\((\S+?)(?:\s+["'][^"']*["'])?\)/g, (m, alt, url) => {
        ...
        // 单元格内链接 markdown → <a>（v1.8.20）。表格占位绕过 cleanNode，故链接文字必须转义、协议白名单
        cellHtml = this._mdLinksToHTML(cellHtml, true);
        // 单元格内加粗/斜体（v1.11.1）：表格标记块在主串 markdown 转换前已被替换为占位符，
        // 因此加粗/斜体必须在此就地转换，否则表格里的 **粗**/*斜* 保存后渲染不出来。
        cellHtml = cellHtml.replace(/\*\*([^*]+?)\*\*/g, '<b>$1</b>');
        cellHtml = cellHtml.replace(/\*([^*]+?)\*/g, '<i>$1</i>');
  ...
  isSkippableElement(element) {
    ...
    // 插件自身 UI 容器（悬浮 tooltip/备注卡片/重要笔记宿主）不参与高亮：备注里含关键词也不应被高亮（v1.10.18）
    if (element.hasAttribute && element.hasAttribute('data-kh-ext-ui')) return true;
  ...
  isHighlightableNode(node) {
    ...
    if (node.nodeType === Node.TEXT_NODE && node.textContent.trim()) {
      ...
      while (parent) {
        ...
        // 插件自身 UI 容器（悬浮 tooltip/备注卡片/重要笔记宿主）：整棵子树不参与高亮（v1.10.18）
        if (parent.hasAttribute && parent.hasAttribute('data-kh-ext-ui')) return false;
  ...
  buildMatchRegex(keyword) {
    ...
    if (keyword.wholeWord) {
      ...
      // 整个模式用 (?:...) 包裹再加词边界，避免 useRegex 含 | 时边界只套在首/末分支（如 A|B|C → B 无边界而漏命中）
