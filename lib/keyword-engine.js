...
const KeywordEngine = {
  ...
  _rowsToTableHtml(rows, label) {
    ...
    // v1.7.4: 解析为「一级分组(位包) + 二级标题」两级结构。
    // 一级分组标题 = 无tab、非label行、非已知标题词、非编号内容行，且紧邻下一行是 tab标题行 或 已知标题词行
    // （如 "32位包:" 下接 "运营备注\t…"、"64位包:" 下接 "基本信息\t…"）。
    ...
    // 过滤有效块：仅保留子块内有内容的块
    const filled = blocks.map(b => ({ gtitle: b.gtitle, subs: b.subs.filter(s => s.lines.length > 0) }))
                        .filter(b => b.subs.length > 0);
    if (!filled.length) return '';
    const total = filled.reduce((n, b) => n + (b.gtitle != null ? 1 : 0) + b.subs.reduce((m, s) => m + s.lines.length, 0), 0);
    const hasGTitle = filled.some(b => b.gtitle != null);
    // 存在非空二级标题才渲染标题列；无标题（如单值）省略标题列，只留「标签列 + 内容列」（v1.7.2）
    ...
    for (const b of filled) {
      if (b.gtitle != null) {
        ...
        if (firstTr) {
          html += '<tr>' + (label != null ? '<td rowspan="' + total + '">' + esc(label) + '</td>' : '') +
