  ...
  _highlightCellVerify(cell, kw, style, job) {
    ...
    if (opt.exact) {
      for (let k = 0; k < tns.length; k++) {
        ...
        if (L) this._wrapVerifyText(tns[k], [{ start: 0, end: L }], color, kw, style, { hitText: hitText });
      }
      return;
    }
    ...
    for (let k = 0; k < tns.length; k++) {
      ...
      if (matches.length) { anyLocal = true; this._wrapVerifyText(tns[k], matches, color, kw, style, { hitText: hitText }); }
    }
  ...
  setupMutationObserver(keywords, config) {
    ...
    const flush = () => {
      ...
      if (roots.length && compiled.length) {
        ...
        // v1.13.0【问题2 兜底】整表/整行刷新常为分批填充，首轮重建可能遇右格值未就绪导致组合词未生效；
        // 对仍停留在文档的受影响容器安排一次延迟二次重扫（幂等），覆盖右格晚到/跨批填充场景。
        this._schedulePostRebuild(roots);
      }
      // ② 再增量处理新增节点
      if (nodes.length) {
        if (nodes.length > 200) {
    ...
    // v1.13.0【问题2】整表/整行刷新后的延迟二次重扫兜底（幂等，覆盖右格值分批/晚到导致组合词未生效）
    const _schedulePostRebuild = (roots) => {
      if (!roots || !roots.length) return;
      const alive = roots.filter(function (r) { return r && document.contains(r); });
      if (!alive.length) return;
      if (this._postRebuildTimer) clearTimeout(this._postRebuildTimer);
      this._postRebuildRoots = alive.slice();
      this._postRebuildTimer = setTimeout(() => {
        this._postRebuildTimer = null;
        const rr = this._postRebuildRoots || [];
        this._postRebuildRoots = [];
        const cmp = this._compileKeywords(keywords, config);
        if (!cmp.length) return;
        for (const r of rr) {
          if (!document.contains(r)) continue;
          const txt = (r.textContent || '').trim();
          if (!txt) continue;
          this._highlightInRoot(r, cmp, config); // 幂等(_clearPlainHitsForTextNode/_clearComboHitsForTextNode)
        }
        if (typeof this.onHighlight === 'function') {
          try { this.onHighlight(); } catch (e) { /* 忽略 */ }
        }
      }, 350);
    };
    const requestFlush = () => {
      ...
      for (const mutation of mutations) {
        ...
        if (mutation.type === 'characterData') {
          ...
          if (parent && parent !== document.body && parent.tagName && !isEditableContext(parent)) {
            // v1.13.0【2修复】表格行(TR)内文本被改写 → 强制「先清后建」：清除该行各格的 _cellVerified，
            // 使组合词左格标题重新参与验证（否则 _cellVerified 记忆「该格已处理」→ 值改写后组合词不恢复）
            const target = resolveUnit(parent);
            if (target && target.tagName === 'TR') {
              if (dirtyRoots.indexOf(target) === -1) dirtyRoots.push(target);
            } else if (hasResidualMarks(target)) {
              if (dirtyRoots.indexOf(target) === -1) dirtyRoots.push(target);
        ...
        for (const node of mutation.addedNodes) {
          if (node.nodeType === Node.ELEMENT_NODE && !Utils.isSkippableElement(node)) {
            ...
            if (parent && parent !== document.body && parent.tagName && !isEditableContext(parent)) {
              // v1.13.0【2修复】同 characterData：TR 内文本改写强制先清后建，清 _cellVerified 保证组合词重验
              const target = resolveUnit(parent);
              if (target && target.tagName === 'TR') {
                if (dirtyRoots.indexOf(target) === -1) dirtyRoots.push(target);
              } else if (hasResidualMarks(target)) {
