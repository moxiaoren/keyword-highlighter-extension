...
const KeywordEngine = {
  ...
  _rowsToTableHtml(rows, label) {
    ...
    // 解析规则（v1.6.43）：\t 分列、\n 分行，渲染成规整表格；「描述」作为左列合并所有行
    const grid = textLines.map(t => String(t).split('\t').map(x => x.trim()));
    let nCols = 1;
    for (const r of grid) if (r.length > nCols) nCols = r.length;
    const total = grid.length;
    let html = '<table class="kh-table">';
    for (let i = 0; i < total; i++) {
      html += '<tr>';
      if (label != null && i === 0) html += '<td rowspan="' + total + '">' + esc(label) + '</td>';
      for (let c = 0; c < nCols; c++) html += '<td>' + esc(grid[i][c] || '') + '</td>';
      html += '</tr>';
    }
    html += '</table>';
