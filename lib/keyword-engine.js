...
const KeywordEngine = {
      ...
      if (m.keyword.effectiveImportant) {
        ...
        for (const f of fetched) {
          ...
          if (f.rows.length === 1 && f.rows[0].length === 1) {
            part = f.label + '：' + f.rows[0][0];               // 单值：描述：xxx
          } else {
            // 多格/多行：输出为 markdown 表格行，悬浮窗渲染成 Excel 表格（v1.6.38）
            part = f.label + '：';
            for (const line of f.rows) part += '\n| ' + line.join(' | ') + ' |';
          }
          note = note ? note + '\n' + part : part;
        }
  ...
  _collectRightBlock(cell, table) {
    ...
    const rows = [];
    const n = Math.min(rowIdx + rowspan, table.rows.length);
    for (let r = rowIdx; r < n; r++) {
      const cells = table.rows[r].cells;
      // 首行从标签右邻格开始；后续行因 rowspan 占位，从同列开始
      const start = (r === rowIdx) ? col + 1 : col;
      const line = [];
      for (let c = start; c < cells.length; c++) {
        const v = (cells[c].textContent || '').trim();
        if (v) line.push(v);
      }
