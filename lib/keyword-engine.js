...
const KeywordEngine = {
  ...
  async highlightKeywords(keywords, config) {
    ...
    const compiledKeywords = keywords.map(kw => {
      ...
      if (!re) return null;
      // 组合关键词翻转(v1.8.3)：kw.text=右格内容(核心)、kw.cellVerify=左格标题。
      // 用「标题正则」在左格定位标题；验证右格含核心(text)后，再于右格高亮核心（标题本格不高亮）。
  ...
  _highlightInRoot(root, compiledKeywords, config) {
    ...
    for (let i = 0; i < verifyJobs.length; i++) {
      ...
      else if (job.tn) this._highlightFakeCellVerify(job.tn, job.kw, job.style);
    }
