  ...
  setupMutationObserver(keywords, config) {
    ...
    const hasResidualMarks = (el) => {
      ...
      for (let i = 0; i < hits.length; i++) {
        const m = hits[i];
        if (!m.combo) continue;
        // v1.13.3【行引用识别】：优先用命中所属行(rowRef)判断是否落在本处理单元——即便该行文本节点
        // 已被改写/替换而脱离文档（提交/刷新场景），只要该行曾命中组合词即视为残留，触发整行先清后建
        // 重验，修复「值改回后组合词不重验而永久丢失」。
        const rr = m.rowRef || m.textNode;
        if (!rr || !rr.parentNode) continue;
        if (rr === el) return true;
        let inside = false;
        let e = rr.parentNode;
        while (e && e !== document.body) { if (e === el) { inside = true; break; } e = e.parentNode; }
        if (inside) return true;
        try { if (el.contains && el.contains(rr)) return true; } catch (e2) { /* ignore */ }
      }
      return false;
    };
