...
const KeywordEngine = {
   ...
   */
  // 全量重刷（v1.10.9 回退）：MutationObserver 监听整个页面（document.body + subtree），
  // 只要页面新增任何节点，300ms 静默后即全量重刷整篇 document.body。
  // 放弃 v1.10.4~1.10.8 的增量高亮 / 行级重扫 / 区域「先清后建」复杂逻辑——它们在动态页面上
  // 会误把刚生成的高亮/重要笔记标记当作残留先清掉（"先高亮后消失"），
  // 回退到最朴素的"新增即全量重刷"：依赖 data-kh-highlighted 幂等跳过，removeAllHighlights 可恢复。
      ...
      for (const mutation of mutations) {
        ...
        for (const node of mutation.addedNodes) {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            // 新元素内部尚未高亮过 → 需要一次全量重刷
            if (!node.querySelector('[data-kh-highlighted]')) { shouldHighlight = true; break; }
          } else if (node.nodeType === Node.TEXT_NODE) {
            // textContent 赋值会新增文本节点（异步填充默认值→真实值的常见方式）
            // 也属"新增节点"，纳入全量重刷触发，覆盖「值后到」场景。
            const p = node.parentElement;
            if (p && p !== document.body && p.tagName) { shouldHighlight = true; break; }
          }
        }
