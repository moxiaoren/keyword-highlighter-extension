...
const KeywordEngine = {
      ...
      if (m.keyword.effectiveImportant) {
        ...
        for (const f of fetched) {
          ...
          if (f.rows.length === 1 && f.rows[0].length === 1 && !/[\r\n]/.test(f.rows[0][0].t)) {
            ...
            // 多格/多行块：生成带合并或按行的 Excel 表格
            part = f.label + '：' + this._rowsToTableHtml(f.rows);
          }
          note = note ? note + '\n' + part : part;
        }
   ...
   */
  _rowsToTableHtml(rows) {
    const esc = (s) => { const d = document.createElement('div'); d.textContent = (s == null ? '' : String(s)); return d.innerHTML; };
    let html = '<table class="kh-table">';
    for (const line of rows) {
      // 单格且含换行 → 大单元格内多行文本，按行拆成多行单列（v1.6.40）
      if (line.length === 1 && /[\r\n]/.test(line[0].t)) {
        const parts = String(line[0].t).split(/\r?\n/).map(x => x.trim()).filter(Boolean);
        for (const ln of parts) html += '<tr><td>' + esc(ln) + '</td></tr>';
        continue;
      }
      html += '<tr>';
      for (const c of line) {
