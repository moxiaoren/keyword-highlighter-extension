...
const KeywordEngine = {
  ...
  _highlightTextNode(textNode, compiledKeywords, config) {
    ...
    for (const m of filtered) {
      ...
      if (m.keyword.cellVerifyEnabled && m.keyword.cellVerify) {
        ...
        // 同时把原网页右侧相邻单元格（期望值）用关键词颜色高亮，并把组合关键词的悬浮备注/卡片交互挂到期望值上（v1.7.10）
        this._highlightCellVerify(textNode, m.keyword, config.highlightStyle);
      }
   ...
   */
  _highlightCellVerify(textNode, kw, style) {
