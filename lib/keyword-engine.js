...
const KeywordEngine = {
  ...
  _highlightCellVerify(cell, kw, style, job) {
    ...
    // v1.13.1【值后到根治】不在函数开头立即 add(kid)。原来无论是否命中都先标「已处理」，
    // 导致右侧单元格初值空（值后到/异步填充/刷新未就绪）时该格该词被永久跳过 → 组合词不命中。
    // 改为：仅当确实产生命中高亮后才缓存「已处理」，未命中时允许后续值后到重新验证。
    const markDone = () => { if (kid) doneSet.add(kid); };
    ...
    for (let k = 0; k < tns.length; k++) {
      ...
      if (matches.length) { anyLocal = true; this._wrapVerifyText(tns[k], matches, color, kw, style, { hitText: hitText }); markDone(); }
    }
