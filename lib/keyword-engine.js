...
const KeywordEngine = {
  ...
  getPlainHits() {
    ...
    for (const h of this._plainHits) {
      ...
      out.push({
        text: (textNode.nodeValue || '').slice(h.start, h.end),
        range: h.range,
        textNode: textNode,
  ...
  _highlightCellVerify(cell, kw, style) {
    ...
    if (!anyLocal) {
      ...
      if (cross.length) {
        ...
        byNode.forEach((mArr, tn) => { this._wrapVerifyText(tn, mArr, color, kw, style); });
      }
    }
  },
  ...
  _wrapVerifyText(tn, matches, color, kw, style) {
    ...
    for (let k = 0; k < matches.length; k++) {
      ...
      // 重要标识数据（置顶悬浮笔记聚合用），与普通词一致
      if (kw && kw.effectiveImportant) {
        meta.important = true;
