      ...
      for (const mutation of mutations) {
        ...
        if (mutation.type === 'characterData') {
          ...
          if (parent && parent !== document.body && parent.tagName) {
            // 区域刷新信号②：改写的目标文本常位于高亮 span 内，故上溯到所在 td/th 单元格判断
            const cell = parent.closest ? parent.closest('td,th') : null;
            const target = cell || parent;
            if (hasResidualMarks(target)) {
              scheduleRebuild(target); // 复用节点含旧跨格/抓取标记 → 整体先清后建
            } else if (!isEditableContext(parent) && !seen.has(parent)) {
              seen.add(parent);
              pendingNodes.push(parent); // 普通内容改写：局部增量
            }
          }
          continue;
        }
        ...
        for (const node of mutation.addedNodes) {
          if (node.nodeType === Node.ELEMENT_NODE && !Utils.isSkippableElement(node)) {
            ...
            if (parent && parent !== document.body && parent.tagName && !isEditableContext(parent) && !seen.has(parent)) {
              if (hasResidualMarks(parent)) { scheduleRebuild(parent); continue; }
              seen.add(parent);
