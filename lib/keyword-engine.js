...
const KeywordEngine = {
  ...
  _highlightTextNode(textNode, compiledKeywords, config, verifyJobs) {
    ...
    for (const m of filtered) {
      ...
      if (m.keyword.effectiveImportant) {
        ...
        // v1.10.6：单独存「预设笔记」部分，供异步重抓时与最新抓取字段拼接
        span.setAttribute('data-kh-important-base', m.keyword.effectiveImportantNote || '');
        if (m.keyword.effectiveImgSize) span.setAttribute('data-kh-important-img-size', m.keyword.effectiveImgSize);
        if (m.keyword.effectiveImpNoteBg) span.setAttribute('data-kh-important-bg', m.keyword.effectiveImpNoteBg); // v1.9.3 笔记底色(含分组兜底)
      }
  ...
  _highlightCellVerify(cell, kw, style) {
    ...
    // 跳过已嵌在本插件验证高亮 span 内的文本节点（同一格被多个词命中时，避免跨词重复包裹/嵌套后格文本）
    const all = this._cellTextNodes(cell);
    const tns = [];
    for (let i = 0; i < all.length; i++) {
      ...
      let wrapped = false;
      while (p && p !== cell) {
        if (p.hasAttribute && p.hasAttribute('data-kh-cell-verify-hi-span')) { wrapped = true; break; }
        p = p.parentElement;
      }
      if (!wrapped) tns.push(tn);
    }
    ...
    for (let k = 0; k < matches.length; k++) {
      ...
      if (kw) {
        ...
        if (kw.effectiveImportant && !span.hasAttribute('data-kh-important')) {
          ...
          if (note) span.setAttribute('data-kh-important-note', note);
          if (kw.effectiveImgSize) span.setAttribute('data-kh-important-img-size', kw.effectiveImgSize);
  ...
  setupMutationObserver(keywords, config) {
    ...
    // 批次累积 + 防抖：在静默窗口内合并同一时段的节点，一次性增量处理（与旧 _highlightTimer 语义一致，destroy 可取消）
    let pendingNodes = [];
    // v1.10.7：区域「先清后建」容器集合（换页/刷新残留修复）
    let rebuildRoots = [];
    ...
    const flush = () => {
      ...
      pendingNodes = [];
      // 若节点落在待重建容器内，交给区域重建处理即可，避免增量与重建重复扫
      if (rebuildRoots.length) {
        nodes = nodes.filter(function (n) {
          return !rebuildRoots.some(function (r) { return r !== n && r.contains && r.contains(n); });
        });
      }
      if (!nodes.length) return;
      // 单次出现极大量新增（近似整页替换/全量渲染）时退化为一次全量，保证正确性
      if (nodes.length > 200) {
    ...
    // 区域「先清后建」：对发生刷新/替换/复用改写的容器清掉旧高亮标记再全量重扫，
    // 根治换页后「上一页高亮/抓取值残留到下一页同位置」的问题（v1.10.7）。
    const doRebuildRoots = () => {
      this._rebuildTimer = null;
      const roots = rebuildRoots;
      rebuildRoots = [];
      if (!roots.length) return;
      const compiled = this._compileKeywords(keywords, config);
      if (!compiled.length) return;
      for (const root of roots) {
        if (!document.contains(root)) continue;      // 容器已被整体替换/移除 → 无需重建
        this._removeHighlightsInRoot(root);           // ① 先清旧高亮/跨格/抓取标记
        this._highlightInRoot(root, compiled, config); // ② 重新全量扫描
      }
      if (typeof this.onHighlight === 'function') {
        try { this.onHighlight(); } catch (e) { /* 忽略面板刷新异常 */ }
      }
    };
    const scheduleRebuild = (root) => {
      if (rebuildRoots.indexOf(root) === -1) rebuildRoots.push(root);
      if (this._rebuildTimer) clearTimeout(this._rebuildTimer);
      this._rebuildTimer = setTimeout(doRebuildRoots, 400); // 静默后统一重建，避免换页过程反复重建
    };

    // 某元素内是否残留本插件的「跨格/抓取」旧标记（组合词期望值 / 仅抓取 / 重要笔记抓取值）。
    // 这些标记由增量(只处理新增、幂等跳过)天然清不掉，是换页残留的根源 → 命中则改走区域重建。
    // 兼容 el 自身带标记（如组合词期望值 span）与其后代带标记两种情况。
    const hasResidualMarks = (el) => {
      if (!el) return false;
      const sel = '[data-kh-cell-verify-hi-span],[data-kh-important-note],[data-kh-fetch-only],[data-kh-important]';
      try {
    ...
    // v1.10.7：值刷新——任何 DOM 变化(含字符改写/移除)都调度一次异步重抓重要笔记抓取字段，
    // 与高亮引擎解耦、不受编辑区防护影响；待 DOM 静默后再抓，避免抓到「填入中间态」的默认值。
    ...
    // 判断某个元素是否处于可编辑/输入上下文中（input、富文本编辑器等），用于跳过高频字符变化（v1.10.6）
    ...
    this.observer = new MutationObserver((mutations) => {
      ...
      for (const mutation of mutations) {
        // 区域刷新信号①：旧节点被移除（换页/翻番/列表刷新必然发生）。
        // 排除本插件高亮 replaceChild 产生的「文本节点/本插件 span」移除，避免自触发死循环。
        ...
        // 文本内容被改写（异步填充/复用节点更新文本）
        if (mutation.type === 'characterData') {
          const parent = mutation.target && mutation.target.parentElement;
          if (parent && parent !== document.body && parent.tagName) {
            // v1.10.8：重扫范围从「所在单元格」升级为「所在行(tr)」。
            // 组合词（单元格验证）匹配的是左格标题文本，右格值后到（默认值→真实值）时
            // 只扫右格永远扫不到左格标题，首扫验证失败的命中无法补救（高亮/重要笔记缺失）。
            // 升级到行级后左格标题重新参与扫描、验证重新执行；行内文本节点有限，开销可控。
            const cell = parent.closest ? parent.closest('td,th') : null;
            const row = cell && cell.closest ? cell.closest('tr') : null;
            const target = row || cell || parent;
            if (hasResidualMarks(target)) {
              scheduleRebuild(target); // 复用节点含旧跨格/抓取标记 → 整行先清后建
            } else if (!isEditableContext(parent) && !seen.has(target)) {
              seen.add(target);
              pendingNodes.push(target); // 普通内容改写：行级增量重扫（含左格标题，验证可补救）
            }
          }
          continue;
        }
        ...
        for (const node of mutation.addedNodes) {
          if (node.nodeType === Node.ELEMENT_NODE && !Utils.isSkippableElement(node)) {
            ...
            // textContent 赋值会移除旧文本节点并新增文本节点；取父元素做局部重高亮
            // v1.10.8：同 characterData 分支，重扫范围升级到「所在行(tr)」，
            // 让左格标题重新参与组合词验证（值替换是异步更新真实值的常见方式）
            const parent = node.parentElement;
            if (parent && parent !== document.body && parent.tagName && !isEditableContext(parent)) {
              const cell = parent.closest ? parent.closest('td,th') : null;
              const row = cell && cell.closest ? cell.closest('tr') : null;
              const target = row || cell || parent;
              if (hasResidualMarks(target)) { scheduleRebuild(target); continue; }
              if (!seen.has(target)) { seen.add(target); pendingNodes.push(target); }
            }
          }
        }
      }
      // 值刷新：只要发生 DOM 变化就调度异步重抓（问题一修复）
      scheduleRefetch();
      if (pendingNodes.length && !this._flushTimer) {
        this._flushTimer = setTimeout(flush, 300);
      }
      if (rebuildRoots.length && !this._rebuildTimer) {
        this._rebuildTimer = setTimeout(doRebuildRoots, 400);
      }
    });
