...
const KeywordEngine = {
  ...
  _compileKeywords(keywords, config) {
    ...
    const compiledKeywords = keywords.map(kw => {
      ...
      // v1.50.0：上下格(tb)组合词需行关键词(text)正则 kw.regex（左右格 lr 的 text 由第二遍 right-cell 匹配，无需此处正则；罕见字不做 tb）
      const isTbCombo = isCombo && kw.comboAxis === 'tb';
      const re = isCombo ? (isTbCombo && !isRare ? Utils.buildMatchRegex(kw) : null) : (isRare ? null : Utils.buildMatchRegex(kw));
      ...
      return {
        ...
        // v1.14.0【2】重要笔记底色：不再单独配色；关键词或所属分组勾了「复用高亮底色」则铺该词实际高亮底色（分组色>词色>默认），否则不铺
        effectiveImpNoteBg: !!(kw.impNoteUseHlColor || (group && group.important && group.impNoteUseHlColor))
          ? ((group && group.bgColor) || kw.bgColor || config.highlightStyle.defaultBgColor) : '',
  ...
  _highlightInRoot(root, compiledKeywords, config) {
    ...
    for (let i = 0; i < verifyJobs.length; i++) {
      ...
      if (job.tbCol) this._highlightComboTbCol(job.tbCol, job.kw, job.style, job);
      else if (job.cell) this._highlightCellVerify(job.cell, job.kw, job.style, job);
   ...
   */
  /**
   * v1.14.1【3】跨文本节点词边界判定：全词命中若贴节点首/尾，检查相邻兄弟节点是否延续词字符。
   * 命中恰好抵达节点末尾 → 看右邻兄弟首个可见字符；命中始于节点开头 → 看左邻兄弟末个可见字符。
   * 相邻兄弟首个/末个字符是词字符(\p{L}\p{N}_) → 该命中其实处于一个被拆开的词的词中/词尾，返回 true（作废命中）。
   */
  _crossNodeBoundaryIsWord(tn, start, end) {
    const text = tn.nodeValue || '';
    const isWordChar = (ch) => !!ch && /[\p{L}\p{N}_]/u.test(ch);
    if (end === text.length) {
      const n = this._nextVisibleNode(tn);
      const ch = this._edgeChar(n, false);
      if (isWordChar(ch)) return true;
    }
    if (start === 0) {
      const p = this._prevVisibleNode(tn);
      const ch = this._edgeChar(p, true);
      if (isWordChar(ch)) return true;
    }
    return false;
  },
  _nextVisibleNode(node) {
    let n = node.nextSibling;
    while (n) { if (this._visibleText(n)) return n; n = n.nextSibling; }
    return null;
  },
  _prevVisibleNode(node) {
    let p = node.previousSibling;
    while (p) { if (this._visibleText(p)) return p; p = p.previousSibling; }
    return null;
  },
  _visibleText(n) {
    if (!n) return false;
    ...
    if (n.nodeType === Node.ELEMENT_NODE) {
      if (n.nodeName === 'SCRIPT' || n.nodeName === 'STYLE' || n.nodeName === 'BR') return false;
      return !!(n.textContent || '').trim();
    }
    return false;
  },
  // isLast=true 取可见末字符，否则取可见首字符（忽略首尾空白）
  _edgeChar(n, isLast) {
    if (!n) return '';
    const raw = n.nodeType === Node.TEXT_NODE ? (n.nodeValue || '') : (n.textContent || '');
    if (isLast) {
  ...
  // ===== v1.50.0 上下格组合词（列关键词×行关键词）=====
  // 表头列关键词命中后，定位该列【数据区】所有单元格（每行一个），供第二遍行关键词匹配。
  _locateColumnForHeader(tn, kw) {
    const headerCell = tn.parentNode && tn.parentNode.closest ? tn.parentNode.closest('td,th') : null;
    if (!headerCell) return [];
    const table = headerCell.closest('table');
    if (!table) return [];
    const headerRow = headerCell.closest('tr');
    if (!headerRow) return [];
    // v1.50.0 仅表头生效：列关键词命中的必须是表头单元格（其标签为 TH，或该单元格所在行就是表头行），
    // 避免数据区某行碰巧含列关键词文本时误触发 tb（只允许“列关键词在表头定列”）。
    if (headerCell.tagName !== 'TH') {
      const hdrRow = this._firstHeaderRow(table);
      if (!hdrRow || headerRow !== hdrRow) return [];
    }
    const colIndex = Array.prototype.indexOf.call(headerRow.cells, headerCell);
    if (colIndex < 0) return [];
    // 同表数据列（排除表头行）
    const rows = this._dataRows(table, headerRow);
    const cells = rows.map(r => (r.cells && r.cells[colIndex]) || null).filter(Boolean);
    if (cells.length) return cells;
    // 同表无数据行（表头/数据区分成两个独立 <table>）：跨表定位该列数据单元格。
    return this._findColumnInOtherTable(headerCell, colIndex, kw);
  },
  _dataRows(table, headerRow) {
    const rows = [];
    const push = (r) => { if (r && r !== headerRow) rows.push(r); };
  ...
  _firstHeaderRow(table) {
    if (table.tHead && table.tHead.rows && table.tHead.rows.length) return table.tHead.rows[0];
    if (table.rows && table.rows.length) return table.rows[0];
    return null;
  },
  // v1.50.0 el-table 支持：从表头向上爬到「含 ≥2 个 <table>」的最近公共祖先
  // （Element UI 固定表头 = 表头表 + 数据表两个独立 <table>，同在一个 div.el-table__inner-wrapper 内）。
  // 这样表头与数据区分离成两表的组件（el-table 等）也能定位到数据列；无此结构时退回 document.body。
  _findColumnScope(headerCell) {
    ...
    return body; // 页面本身含多表 / 表头表就是唯一表时，用全局扫描兜底
  },
  // 跨表：优先按“列名一致 / 列关键词命中列表头”定位列；找不到时回退按列序号对齐。
  _findColumnInOtherTable(headerCell, colIndex, kw) {
    const table = headerCell.closest('table');
    const colName = (headerCell.textContent || '').trim();
    const scope = this._findColumnScope(headerCell);
    // B 优先：按框架结构特征（el-table__body / body-wrapper 兄弟）精确定位数据表，规避无关表列序号误取。
    const bodyTable = this._findElTableBody(headerCell, table, scope);
    if (bodyTable) {
      const cells = this._columnTdCells(bodyTable, colIndex);
      if (cells.length) return cells;
    }
    // A 通用回退：遍历祖先/全局表，按“列名一致 → 列关键词命中表头 → 列序号对齐”定位。
    const tables = scope.querySelectorAll('table');
    for (let t = 0; t < tables.length; t++) {
      const tbl = tables[t];
      if (tbl === table || tbl.contains(headerCell)) continue;
      let idx = -1;
      const head = this._firstHeaderRow(tbl);
      if (head) {
        idx = Array.prototype.findIndex.call(head.cells, (c) => {
          const t2 = (c.textContent || '').trim();
          if (t2 && colName && t2 === colName) return true;
          return !!t2 && this._cellTextMatches(cell => cell.textContent, t2, kw.cellVerify, kw.cellVerifyUseRegex, kw.cellVerifyMatchMode);
        });
      }
      if (idx < 0) idx = colIndex; // 回退：两表列序号对齐
      if (idx < 0) continue;
      const cells = this._columnTdCells(tbl, idx);
      if (cells.length) return cells;
    }
    return [];
  },
  // v1.50.0 el-table 结构特征定位数据表（比列序号通用对齐更准，规避无关表误取）：
  // ① 表头所在 el-table__header-wrapper 的兄弟 el-table__body-wrapper 内 <table>；
  // ② 祖先范围内 class 含 el-table__body（且非 header）的 <table>。
  _findElTableBody(headerCell, headerTable, scope) {
    let headerWrapper = headerCell.closest('.el-table__header-wrapper');
    ...
    if (scope && scope.querySelectorAll) {
      ...
      for (let i = 0; i < ts.length; i++) {
        ...
        if (cls.indexOf('el-table__body') >= 0 && cls.indexOf('header') < 0) return t;
      }
    }
    return null;
  },
  // 取某表某列的 td 单元格（只取 td，自然跳过表头 th）。
  _columnTdCells(table, colIdx) {
    const cells = [];
    for (let i = 0; i < table.rows.length; i++) {
      const td = table.rows[i].children[colIdx];
      if (td && td.tagName === 'TD') cells.push(td);
    }
    return cells;
  },
  _cellTextMatches(getText, text, keyword, useRegex, matchMode) {
    const t = (text || '').trim();
    if (!t || !keyword) return false;
    const k = String(keyword);
    if (matchMode === 'exact') return t === k;
    if (useRegex) { try { return new RegExp(k, 'i').test(t); } catch (e) { return t.indexOf(k) >= 0; } }
    return t.indexOf(k) >= 0;
  },
  // 第二遍：对目标列数据单元格，用行关键词(kw.regex)在每个文本节点上匹配并高亮『命中片段』。
  _highlightComboTbCol(cells, kw, style, job) {
    const rowRegex = kw.regex;
    if (!rowRegex) return 0;
    if (!this._tbColProcessed) this._tbColProcessed = new WeakMap();
    const bg = kw.effectiveBgColor || (style && style.defaultBgColor) || '#ff9500';
    const textColor = kw.effectiveTextColor || (style && style.defaultTextColor) || '#000';
    let count = 0;
    for (let c = 0; c < cells.length; c++) {
      const cell = cells[c];
      if (!cell || cell.nodeType !== Node.ELEMENT_NODE) continue;
      if (!cell.textContent || !cell.textContent.trim()) continue; // 图片/空单元格无文字
      // 每单元格每词只“命中后才标记”一次（v1.50.0）：空/占位/未命中单元格不写入缓存，
      // 数据值后到/异步填充时可再次进入匹配补高亮；命中才标记防同列多表头命中重复计数/重复注册。
      let done = this._tbColProcessed.get(cell);
      if (done && done.has(kw.id)) continue;
      if (!done) { done = new Set(); this._tbColProcessed.set(cell, done); }
      // 重建前往：清该单元格内旧的本词 tb 命中
      this._clearComboHitsInElement(cell, kw.id);
      let cellHit = 0;
      const walker = document.createTreeWalker(cell, NodeFilter.SHOW_TEXT, {
        acceptNode: (n) => Utils.isHighlightableNode(n) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT
      });
      let tn;
      while ((tn = walker.nextNode())) {
        ...
        while ((m = rowRegex.exec(t)) !== null) {
          ...
          // 跨节点词边界（v1.14.1）：全词命中贴节点首尾须检查相邻兄弟延续字符
          if (kw.wholeWord && this._crossNodeBoundaryIsWord(tn, start, end)) {
            if (m[0].length === 0) rowRegex.lastIndex++;
            continue;
          }
          let range = null;
          try { range = document.createRange(); range.setStart(tn, start); range.setEnd(tn, end); } catch (e) { range = null; }
          const meta = {
            range: range, textNode: tn, start: start, end: end,
            kwId: kw.id || '', bg: bg, textColor: textColor,
            note: kw.note || '', important: false, importantNote: '', importantBase: '',
            imgSize: '', impNoteBg: '', combo: true, adj: kw.cellVerify || ''
          };
          if (kw.effectiveImportant) {
            meta.important = true;
            meta.importantNote = kw.effectiveImportantNote || '';
            meta.importantBase = kw.effectiveImportantNote || '';
            if (kw.effectiveImgSize) meta.imgSize = kw.effectiveImgSize;
            if (kw.effectiveImpNoteBg) meta.impNoteBg = kw.effectiveImpNoteBg;
          }
          if (range) { this._registerPlainHit(meta); count++; cellHit++; }
          if (m[0].length === 0) rowRegex.lastIndex++;
        }
      }
      if (cellHit > 0) done.add(kw.id); // 命中才标记（值后到可补，未命中不卡死）
    }
    return count;
  },
  // 清除某元素内该 kwId 的 tb(combo) 命中（重建/值变化时先清后建，避免重复注册）。
  _clearComboHitsInElement(el, kwId) {
    const idxs = [];
    for (let i = 0; i < this._plainHits.length; i++) {
      const m = this._plainHits[i];
      if (!m.combo || !m.textNode) continue;
      if (kwId && m.kwId !== kwId) continue;
      if (el === m.textNode || el.contains(m.textNode)) idxs.push(i);
    }
    ...
    for (const kw of compiledKeywords) {
      ...
      while ((match = regex.exec(text)) !== null) {
        if (isCombo) {
          ...
          // v1.50.0 上下格(tb)组合词：列关键词(表头)命中 → 收集目标列数据单元格，供第二遍行关键词匹配高亮。
          if (kw.comboAxis === 'tb') {
            if (verifyJobs && Array.isArray(verifyJobs)) {
              const cells = this._locateColumnForHeader(textNode, kw);
              if (cells.length) verifyJobs.push({ tbCol: cells, kw: kw, style: config.highlightStyle, hitText: match[0] ? String(match[0]) : '' });
            }
        ...
        // v1.14.1【3】跨文本节点词边界：全词命中若紧贴节点首/尾，须检查相邻兄弟节点的首/末字符
        // 是否延续词字符（页面把连续文本拆成相邻节点，如 “ 若干 B + <span>UG</span>”），延续则此命中作废。
        if (kw.wholeWord && this._crossNodeBoundaryIsWord(textNode, match.index, match.index + match[0].length)) {
          if (match[0].length === 0) kw.regex.lastIndex++; // 防止死循环
          continue;
        }
        allMatches.push({
   ...
   */
  // v1.50.x 抓取字段图片支持：把 <img> 转成一个“esc 安全、可还原为 <img>”的占位符，
  // 用于抓取链路（_cellText/_cellVisualText → _rowsToTableMulti → KH_TABLE_HTML）里承载图片。
  // src 协议白名单同 sanitize：仅 http(s)（含相对路径转绝对）保留，非法/占位图/null 返回 null。
  _imgPlaceholder(img) {
    if (!img) return null;
    let src = (img.getAttribute && img.getAttribute('src')) || '';
    if (!src) return null;
    if (/^data:image\//i.test(src)) return null;            // 跳过 data: 占位图
    if (/^[a-z][a-z0-9+.-]*:/i.test(src) && !/^https?:/i.test(src) && !/^\/\//i.test(src)) return null; // javascript: 等非法协议
    try { src = new URL(src, location.href).href; } catch (e) { /* 保留原样 */ }
    if (!/^https?:\/\//i.test(src)) return null;            // 只保留绝对 http(s)
    const alt = (img.getAttribute && img.getAttribute('alt')) || '';
    const enc = (s) => encodeURIComponent(s == null ? '' : String(s)).replace(/'/g, '%27');
    return '\u0001KHIMG\u0001' + enc(alt) + '|' + enc(src) + '\u0001ENDIMG\u0001';
  },
  // 把占位符还原成安全 <img>（在 _rowsToTableMulti 的 esc 转义后调用，正文仍转义、图片安全插入）。
  // 不携带宽高，尺寸统一交由重要笔记 .khin-item-body img 的 --kh-img-size 缩略控制。
  _restoreTableImgs(str) {
    const escA = (s) => String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
    return String(str).replace(/\u0001KHIMG\u0001([^|]*)\|([^\u0001]*)\u0001ENDIMG\u0001/g, (_, a, u) => {
      let alt = '', src = '';
      try { alt = decodeURIComponent(a); src = decodeURIComponent(u); } catch (e) { return ''; }
  ...
  _cellText(el, mergeLines) {
    ...
    const walk = (node) => {
      ...
      const tag = (node.tagName || '').toUpperCase();
      if (tag === 'BR') { segs.push('\n'); return; }
      // v1.50.x 抓取字段图片：<img> 转占位符，下游还原为缩略 <img>（不丢失图片）
      if (tag === 'IMG') { const ph = this._imgPlaceholder(node); if (ph) segs.push(ph); return; }
  ...
  _rowsToTableMulti(multi) {
    const esc = (s) => { const d = document.createElement('div'); d.textContent = (s == null ? '' : String(s)); return this._restoreTableImgs(d.innerHTML); };
  ...
  _wrapVerifyText(tn, matches, color, kw, style, opts) {
    ...
    // 【组合词所属行】记录命中所属处理单元(行/表格)：当该行内容被改写/文本节点脱离文档时，据此定位并触发该行先清后建、重验组合词。
    let rowRef = null;
    {
  ...
  _fetchNonEmptyPass(textNode, match, kw) {
    // 仅抓取【触发判据】= 标题词（左格）右侧是否有内容：直接右邻单元格存在且为空 → 不抓取（用户明确）。
    //   不用 _findRightCell（它会跳过空格子找更右的非空格），否则“右格为空”被误判为“无右格”。
    const parentEl = textNode.parentElement;
    const cell = parentEl && parentEl.closest ? parentEl.closest('td,th') : null;
    if (cell && cell.parentElement) {
      const nx = cell.nextElementSibling;
      if (nx && /^TD|TH$/i.test(nx.tagName)) {
        const right = this._cellText(nx, false) || ''; // 取正文（跳过按钮/交互控件），右格有内容才触发
        return /[\p{L}\p{N}]/u.test(right);
      }
    }
    // 无独立右格（假表格/非表格）：配置了抓取字段(fetchLabels)时按其标签能否抓到右邻判定
    const rawFL = kw.fetchLabels && String(kw.fetchLabels).trim();
    if (rawFL) {
      try { return this._extractFetched(textNode, kw).length > 0; } catch (e) { return false; }
    }
    // 无 fetchLabels 且无右格：按 | / Tab 后段判定
    const t = textNode.textContent || '';
    const after = t.slice(match.index + match[0].length);
    const sep = after.match(/[|\t]/);
    const right = sep ? after.slice(sep.index + 1) : '';
    return /[\p{L}\p{N}]/u.test(right);
  },
  ...
  _cellVisualText(cell) {
    ...
    const rows = []; // [{top, text, imgs:[]}]
    const addSeg = (top) => {
      const last = rows[rows.length - 1];
      if (last && Math.abs(last.top - top) <= 8) return last;
      const row = { top: top, text: '', imgs: [] };
    ...
    while ((n = walker.nextNode())) {
      ...
      const row = addSeg(top);
      row.text = row.text ? row.text + ' ' + t : t;
    }
    // v1.50.x 抓取字段图片：收集 <img> 转占位符，按位置并入所在行（纯图单元格也可获得图片）
    const imgs = cell.querySelectorAll('img');
    for (let i = 0; i < imgs.length; i++) {
      const ph = this._imgPlaceholder(imgs[i]);
      if (!ph) continue;
      let top = null;
      try {
        const r = document.createRange(); r.selectNodeContents(imgs[i]);
        top = Math.round(r.getBoundingClientRect().top);
      } catch (e) { top = rows.length * 100; }
      addSeg(top).imgs.push(ph);
    }
    return rows.map(r => (r.imgs.length ? (r.text ? r.text + ' ' : '') + r.imgs.join(' ') : r.text)).filter(Boolean).join('\n');
  },
  ...
  _highlightFetchOnly(tn, kw, style) {
    ...
    // v1.13.6【空值守卫】配置了抓取字段(fetchLabels)但按标签抓不到内容（目标右邻为空 / 无该标签）时，
    //   该条记录视为无内容：【不再抓取、也不回退抓标题右格】——恢复“右格为空则不抓”的语义。
    const rawFL = String(kw.fetchLabels || '').trim();
    try {
      const fetched = this._extractFetched(tn, kw);
  ...
  removeAllHighlights() {
    ...
    if (this._cellVerified) this._cellVerified = new WeakMap();
    // v1.50.0 对称补清 tb 缓存：切标签页切回/开关插件（destroy 路径）也清 _tbColProcessed，
    // 否则 el-table 复用同一批 td 时 tb 被 stale 缓存跳过、切回/重开后不再恢复高亮。
    this._clearTbColProcessed(null);
   ...
   */
  // v1.50.0 统一「tb 组合词已处理缓存」清理出口：避免多条清理路径各自实现导致漏清。
  // root 传入元素 → 移出 root 范围内 td,th 的缓存（增量重建路径，等价 _removeHighlightsInRoot 局部清理）；
  // root 为 null/undefined → 全量重置（destroy/切标签/开关插件路径，removeAllHighlights 调用）。
  _clearTbColProcessed(root) {
    if (root) {
      if (!this._tbColProcessed) return;
      let cells;
      try { cells = root.querySelectorAll('td, th'); } catch (e) { cells = []; }
      for (let i = 0; i < cells.length; i++) this._tbColProcessed.delete(cells[i]);
      const tag = (root.tagName || '').toLowerCase();
      if (tag === 'td' || tag === 'th') this._tbColProcessed.delete(root);
      return;
    }
    if (this._tbColProcessed) this._tbColProcessed = new WeakMap();
  },
  _removeHighlightsInRoot(root) {
    ...
    // v1.50.0 tb 组合词缓存清理（统一出口 _clearTbColProcessed）：重建路径清 root 范围内 td,th 的缓存,
    // 使 tb 在值后到/重建后仍能被重新定位+高亮（与左右格 _cellVerified 的局部清理对齐）。
    this._clearTbColProcessed(root);
