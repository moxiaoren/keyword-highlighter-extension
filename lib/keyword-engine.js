...
const KeywordEngine = {
    ...
    for (const m of filtered) {
      ...
      if (m.keyword.effectiveImportant) {
        ...
        for (const f of fetched) {
          let part;
          if (f.rows.length === 1 && f.rows[0].length === 1 && !/[\r\n]/.test(f.rows[0][0].t)) {
            part = f.label + '：' + f.rows[0][0].t;            // 单值（含合并大格但无换行）：描述：xxx
          } else {
            // 多格/多行块：标签作为左列合并多行，右侧内容成一格一格（v1.6.41）
            part = this._rowsToTableHtml(f.rows, f.label);
          }
          note = note ? note + '\n' + part : part;
        }
  ...
  _rowsToTableHtml(rows, label) {
    ...
    // 展开为单格文本行：规整多列格原样走网格渲染（不分组）
    const textRows = [];
    let gridMode = false;
    for (const line of rows) {
      if (line.length === 1 && /[\r\n]/.test(line[0].t)) {
        const parts = String(line[0].t).split(/\r?\n/).map(x => x.trim()).filter(Boolean);
        textRows.push.apply(textRows, parts);
      } else if (line.length === 1) {
        textRows.push(line[0].t);
      } else {
        gridMode = true;
      }
    }
    if (gridMode) return this._renderGrid(rows, label, esc);
    // 按标题分组（标题单独列，其后内容归该标题）
    const titles = this.FETCH_GROUP_TITLES || [];
    const groups = [];
    let cur = null;
    for (const line of textRows) {
      let matched = null, rest = null;
      for (const t of titles) {
        if (line === t) { matched = t; rest = null; break; }
        if (line.indexOf(t) === 0) {
          const after = line.slice(t.length).trim();
          if (after && (line[t.length] === ' ' || line[t.length] === '\t' || line[t.length] === '：' || line[t.length] === ':')) { matched = t; rest = after; break; }
        }
      }
      if (matched) { cur = { title: matched, content: [] }; if (rest) cur.content.push(rest); groups.push(cur); }
      else if (cur) cur.content.push(line);
      else groups.push({ title: '', content: [line] });
    }
    if (!groups.length) return this._renderGrid(rows, label, esc);
    let total = 0;
    for (const g of groups) total += Math.max(1, g.content.length);
    let html = '<table class="kh-table">';
    let ri = 0;
    for (const g of groups) {
      const n = Math.max(1, g.content.length);
      for (let k = 0; k < n; k++) {
        html += '<tr>';
        if (label != null && ri === 0) html += '<td rowspan="' + total + '">' + esc(label) + '</td>';
        if (k === 0) html += (n > 1 ? '<td rowspan="' + n + '">' : '<td>') + esc(g.title) + '</td>';
        html += '<td>' + esc(g.content[k] || '') + '</td>';
        html += '</tr>';
        ri++;
      }
    }
