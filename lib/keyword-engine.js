...
const KeywordEngine = {
  ...
  _highlightTextNode(textNode, compiledKeywords, config) {
    ...
    for (const m of filtered) {
      ...
      if (m.keyword.cellVerifyEnabled && m.keyword.cellVerify) {
        ...
        // 同时把原网页右侧相邻单元格（期望值）用关键词颜色高亮（v1.7.9）
        this._highlightCellVerify(textNode, m.keyword);
      }
   ...
   */
  _highlightCellVerify(textNode, kw) {
    ...
    const need = (kw && kw.cellVerify) ? String(kw.cellVerify).trim() : '';
    ...
    if (opt.exact) {
      ...
      for (let k = 0; k < tns.length; k++) {
        ...
        if (L) this._wrapVerifyText(tns[k], [{ start: 0, end: L }], color);
      }
      return;
    }
    ...
    for (let k = 0; k < tns.length; k++) {
      ...
      if (matches.length) this._wrapVerifyText(tns[k], matches, color);
    }
  },
  ...
  _collectVerifyMatches(tn, need, opt) {
    ...
    if (opt.rx) {
      try {
        const re = new RegExp(need, opt.cs ? 'g' : 'gi');
        let m;
  ...
  _wrapVerifyText(tn, matches, color) {
    const text = tn.nodeValue;
