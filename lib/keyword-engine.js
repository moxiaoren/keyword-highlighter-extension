...
const KeywordEngine = {
    ...
    for (const m of filtered) {
      ...
      if (m.keyword.effectiveImportant) {
        ...
        for (const f of fetched) {
          // 抓取后续字段统一以表格形式展示，不再区分单行/多行（v1.7.1）
          const part = this._rowsToTableHtml(f.rows, f.label);
          note = note ? note + '\n' + part : part;
        }
  ...
  _collectRightBlock(cell, table) {
    ...
    const end = Math.min(rowIdx + rowspan, nRows);
  ...
  _rowsToTableHtml(rows, label) {
    ...
    if (gridMode) return this._renderGrid(rows, label, esc);
    if (!textLines.length) return '';
    // 解析（v1.6.44）：标题行识别双通道——① 含 tab 的行=「标题+下方/同行内容」；
    // ② 无 tab 但匹配已知固定标题词（基本信息/测试信息/资质信息/运营备注）的行=标题行。
    // 渲染：标签列(rowspan=总行) + 标题列(rowspan=内容行数) + 内容列，三列结构
    const knownTitle = (s) => {
    ...
    for (const ln of textLines) {
      ...
      if (ti >= 0) {
        ...
        // 标题与左列标签同名：视为网页重复标题，仅保留其后内容（避免第二列重复出现同名）
        if (label != null && title === String(label)) {
          if (rest) { if (groups.length) groups[groups.length - 1].lines.push(rest); else groups.push({ title: null, lines: [rest] }); }
          continue;
        }
        groups.push({ title: title, lines: rest ? [rest] : [] });
      } else {
        // 无 tab：识别已知固定标题词 / 跳过与左列标签同名的冗余行 / 否则归入当前标题
        ...
        if (isLabelLine) {
          ...
          groups.push({ title: kt, lines: rest ? [rest] : [] });
        } else if (groups.length) {
          groups[groups.length - 1].lines.push(ln);
        } else {
          groups.push({ title: null, lines: [ln] });
        }
      }
    }
    const filled = groups.filter(g => g.lines.length > 0);
    if (!filled.length) return '';
    const total = filled.reduce((n, g) => n + g.lines.length, 0);
    // 存在非空标题才渲染标题列；无标题（如单值）省略标题列，只留「标签列 + 内容列」，避免冗余空白列（v1.7.2）
    const hasTitle = filled.some(g => g.title != null && g.title !== '');
    let html = '<table class="kh-table">';
    let firstTr = true;
    for (let k = 0; k < filled.length; k++) {
      const g = filled[k];
      const cap = g.title == null ? '' : g.title;
      const rowsThis = g.lines.length;
      for (let i = 0; i < rowsThis; i++) {
        if (firstTr) {
          html += '<tr>';
          if (label != null) html += '<td rowspan="' + total + '">' + esc(label) + '</td>';
          if (hasTitle) html += '<td rowspan="' + rowsThis + '">' + esc(cap) + '</td>';
          html += '<td>' + esc(g.lines[i]) + '</td></tr>';
          firstTr = false;
        } else {
          html += '<tr>' + (i === 0 && hasTitle ? '<td rowspan="' + rowsThis + '">' + esc(cap) + '</td>' : '') + '<td>' + esc(g.lines[i]) + '</td></tr>';
        }
      }
    }
    html += '</table>';
