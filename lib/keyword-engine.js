...
const KeywordEngine = {
  ...
  _highlightCellVerify(cell, kw, style) {
    ...
    if (kw && kw.isRareCombo) {
      ...
      for (let kk = 0; kk < tns.length; kk++) {
        ...
        if (hits.length) {
          this._wrapVerifyText(tns[kk], hits.map(function (h) { return { start: h.index, end: h.index + h.length }; }), color, kw, style);
        }
      }
      return;
    }
  ...
  _highlightFakeCellVerify(tn, kw, style) {
    ...
    if (kw.isRareCombo) {
      ...
      if (hits.length) {
        this._wrapVerifyText(tn, hits.map(function (h) { return { start: base + h.index, end: base + h.index + h.length }; }), color, kw, style);
      }
      return;
    }
